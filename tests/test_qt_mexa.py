...
class QtMexaTests(unittest.TestCase):
    ...
    def test_only_wormhole_and_lan_are_offered_with_wormhole_default(self):
        window = BridgeWindow()
        tab = MexaTab(self.controller)
        self.addCleanup(window.close)
        self.addCleanup(tab.close)
        ...
        self.assertEqual(window.transport.currentData(), "relay")
        self.assertIn("Wormhole", tab.transport.currentText())
        self.assertEqual(window.transport.currentText(), "Wormhole (outbound WSS)")
        for removed in ("relay_url", "relay_key", "tunnel_provider"):
            self.assertFalse(hasattr(tab, removed))
        self.assertFalse(hasattr(self.controller, "host_provider"))
        self.assertFalse(tab.host.isEnabled())
        self.assertFalse(tab.port.isEnabled())
        ...
        self.assertIn("built-in Wormhole relay", window.listener_label.text())
        ...
        self.assertTrue(tab.host_panel.isHidden())
        self.assertTrue(window.host.isEnabled())
        self.assertFalse(window.relay_url.isEnabled())
        self.assertFalse(window.relay_key.isEnabled())

    def test_lan_connection_choice_survives_disconnect_rebuild_and_reconnect(self):
        with patch("flow_controller.core.mexa_controller.StreamClient") as client:
            tab = MexaTab(self.controller)
            self.addCleanup(tab.close)
            tab.transport.setCurrentIndex(tab.transport.findData("lan"))
            tab.host.setText("192.0.2.10")
            tab.port.setValue(62000)
            tab.token.setText("synthetic-lan-key-" + "a" * 32)
            tab.save_logs.setChecked(False)
            tab._connect()
            self.assertEqual(self.controller.settings["transport"], "lan")
            self.assertFalse(tab.transport.isEnabled())
            self.controller.disconnect_bridge()
            rebuilt = MexaTab(self.controller)
            self.addCleanup(rebuilt.close)
            self.assertEqual(rebuilt.transport.currentData(), "lan")
            self.assertEqual(rebuilt.host.text(), "192.0.2.10")
            self.assertEqual(rebuilt.port.value(), 62000)
            self.assertTrue(rebuilt.connect_button.isEnabled())
            self.assertFalse(rebuilt.save_logs.isChecked())
            rebuilt._connect()
            self.assertEqual(client.call_count, 2)
            self.assertEqual(client.call_args.args[:3], ("192.0.2.10", 62000, tab.token.text()))
            self.controller.disconnect_bridge()

    def test_wormhole_start_requires_consent_and_passes_only_helper(self):
        tab = MexaTab(self.controller)
        self.addCleanup(tab.close)
        tab.token.setText("synthetic-wormhole-key-" + "a" * 32)
        tab.save_logs.setChecked(False)
        tab.helper_path.setText("C:/trusted/wormhole.exe")
        with patch.object(self.controller, "start_temporary_host") as start:
            tab._start_host()
            start.assert_not_called()
            self.assertIn("Confirm", tab.host_status.text())
